...
| Region | `us-central1` |
| Service name | `invoice-billing-service` |
...
```bash
docker build -t invoice-billing-service:local .
docker run --rm -p 8080:8080 invoice-billing-service:local
```

## Manual Cloud Run Deploy

```bash
IMAGE=us-central1-docker.pkg.dev/probestack-prod/probestack-prod-apps/invoice-billing-service:manual

docker build -t ${IMAGE} .
docker push ${IMAGE}

gcloud run deploy invoice-billing-service \
  --image=${IMAGE} \
  --platform=managed \
  --region=us-central1 \
